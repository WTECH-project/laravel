    ...
    <div class="lg:grid grid-cols-12 gap-4 mt-4">
        ...
        <div class="flex col-span-9 flex-col flex-grow items-center">
            <div class="grid grid-cols-1 gap-4 text-sm sm:text-base md:grid-cols-3 pt-2">
                ...
                <x-products.product link="{{ route('products.show', ['sex_category' => $sex_category, 'product' => $product->id]) }}" brand="{{ $product->brand->name }}" name="{{ $product->name }}" price="{{ $product->price }}" img="{{ $product->images->first()->image_path }}" />
                @endforeach
            </div>
